...
- [ ] GPNV 헤더 체크섬 정확한 알고리즘 (바이트합/CRC32/Fletcher32/워드합 모두 불일치)
- [x] ~~Brightness 기본값 코드 추적~~ → 0x08279E 확정
- [ ] GPZP 리소스 포맷 분석 (고엔트로피 7.9, 압축 포맷)
- [ ] 설정 블록 (0xC2000) 체크섬 알고리즘
...
| 파일 | 크기 | 내용 | 용도 |
|------|------|------|------|
| 160x80_resource1 | 4KB | 마이크 아이콘 | 녹음 모드 LCD UI |
| 160x80_resource2 | 2KB | "GOODBYE" 텍스트 | 전원 종료 시 LCD |
| 160x80_resource3 | 2KB | "WELCOME" 텍스트 | 전원 켤 때 LCD |
| 320x240_img | 31KB | 회색 테스트 패턴 | GPEncoder 더미 출력 |
| 640x480_img | 30KB | 회색 테스트 패턴 | GPEncoder 더미 출력 |
| unknown_large_0x068c10 | 57KB | ARM 코드 (오탐) | FFD8로 시작하는 ARM 명령어 |
...
```
경로: C:\time.txt (SD 카드 루트)
포맷: "2025-01-01 23:59:59"
```
SD 카드에 time.txt를 넣으면 카메라 시계를 자동 설정.

### 메인 펌웨어 벡터 테이블 (0x012C00)

| 벡터 | 타겟 | 설명 |
|------|------|------|
| Reset | 0x0000003C | 메인 코드 진입점 |
| Undef | 0x00000250 | 미정의 명령 핸들러 |
| SWI | 0x0000025C | 소프트웨어 인터럽트 |
| Prefetch Abort | 0x00000268 | 프리페치 에러 |
| Data Abort | 0x00000274 | 데이터 접근 에러 |
| IRQ | 0x00000280 | 인터럽트 핸들러 |
| FIQ | 0x000002B0 | 빠른 인터럽트 핸들러 |

- 타겟 주소가 작은 값 → RAM에 복사 후 실행 (0x0 기반)
- 부트로더가 SPI Flash → RAM 복사 후 Jump
...
| 0x012-0x07C | 5.7-6.3 | ARM 코드 (메인 펌웨어) |
| 0x07C-0x084 | 4.0-5.5 | JPEG 이미지 + USB 디스크립터 |
| 0x084-0x08C | 5.2-6.4 | GPRSPAK 리소스 + WAV 사운드 |
| 0x08C-0x0A8 | 3.0-4.5 | 폰트 글리프 인덱스 + 메뉴 데이터 |
| 0x0A8-0x0B4 | 6.8-7.7 | GPZP 압축 리소스 (WELCOME/GOODBYE 등) |
| 0x0B4-0x0C2 | 0.0 | 패딩 (zeros) — 56KB 여유 |
| 0x0C2-0x0C5 | 0.2 | 설정 블록 (16개 프로파일) |
| 0x0C5-0x100 | 0.0 | 미사용 (0xFF) — 236KB 여유 |

### GPRSPAK 리소스 테이블 (0x0842C0)

```
Magic: GPRSPAK\x00
리소스 파일 목록:
  PC_CAM.GPZ          — PC 카메라 모드 리소스
  POWER_OFF_LOGO.JPG  — 종료 로고
  POWER_ON_LOGO.JPG   — 부팅 로고
  POWEROFF_AUDIO.WAV  — 종료 사운드
  POWERON_AUDIO.WAV   — 부팅 사운드
  SELECT_SHORT.GPZP   — UI 선택바 (짧은)
  SELECTBAR_LONG.GPZP — UI 선택바 (긴)
  U_DISK.GPZ          — USB 디스크 모드 리소스
```

### GPRS v2 리소스 인덱스 (0x089E00)

- 매직: `GPRS`, 버전: 2
- 문자열 테이블 시작: +0x5E4 (영어 = 0x08A3E4)
- 6개 언어 테이블: +0xC686 ~ +0xFD26 (각 0xAF0 = 2800바이트)
- 언어 테이블 구조: (pixel_width: u32, string_offset: u32) 쌍
- 비영어 문자열은 내장 비트맵 폰트의 글리프 인덱스로 추정

### 다국어 지원

6개 언어 지원 (영어 외 5개). 비영어 문자열은 내장 폰트 인덱스 방식:
- 문자열 데이터가 ASCII가 아닌 단일 바이트 글리프 코드
- 0x08C000~0x0A8000 영역이 폰트 글리프 너비/인덱스 데이터

### 완전 기능 목록

**영상**: 1080P/720P/VGA, Loop Recording (최대 15분), Motion Detection + 감도
**사진**: 12M/8M/2M/1M/VGA, High/Standard/Economy, 연속촬영
**ISP**: Brightness, Exposure, WB, Sharpness, Colour, Saturation, EV 보정 (-2.0~+2.0)
**시스템**: 6개 언어, 자동종료, 볼륨, 포맷, PAL, USB 모드, Reset, 날짜/시간, Flicker
**숨겨진**: SD 업그레이드, time.txt 시계설정, CDROM 모드, 녹음 모드

### SD 업그레이드 중요 수정

**이전 분석 오류 수정**: `JH_5307_test.bin`에서 변경한 위치(0x041E90)는 펌웨어 코드 내 하드코딩된 빌드 문자열이지, 업그레이드 함수가 비교하는 파일 offset 0x16이 아님.

파일 offset 0x16은 GPNV 헤더 데이터(`00 00 ff 00 01 01...`)로, 이미 "2025004291200"과 다름.
→ **원본 펌웨어를 그대로 JH_5307.bin으로 SD에 넣어도 업그레이드가 트리거될 수 있음!**

### 수정 난이도별 분류

**쉬움** (바이트 패치):
- Brightness 기본값: 0x08279E → 0x80
- UVC PU bmControls: 0x0822AA → 0x3F
- 버전 문자열: 0x079E6C
- WAV 사운드 교체 (같은 크기 이하)
...
- 0xB4000~0xC2000: 56KB (zeros)
- 0xC5000~0x100000: 236KB (0xFF)
- **총 ~284KB** 자유 플래시 영역
