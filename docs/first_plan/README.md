...
#### 4-0. 해상도 다운그레이드 — CMP 스와핑 (2026-03-30 완료)

**핵심 문제**: 720p 센서를 1600x1200으로 강제 업스케일 → 대각선 계단 현상.

**해상도 스위치 코드 발견** (0x1FDD0~0x1FDF0):
```
CMP R0, #1  → mode 1: 3264x2448 (8M)
CMP R0, #2  → mode 2: 1600x1200 (2M) ← 카메라 기본
CMP R0, #3  → mode 3: 1280x960 (1M)
fallback    → 640x480 (VGA)
```

**CMP 스와핑**으로 mode 2 진입 시 mode 3(1280x960) 코드를 타도록 변경:
- `0x1FDD0: E3500002 → E35000FF` (CMP R0, #2 → CMP R0, #255: mode 2 건너뜀)
- `0x1FDE0: E3500003 → E3500002` (CMP R0, #3 → CMP R0, #2: mode 2가 mode 3으로)
- GPNV XOR 변경 불필요 (원본 유지로 통과)

| 지표 | mode 2 (1600x1200) | mode 3 (1280x960) | 변화 |
|------|-------------------|-------------------|------|
| 업스케일 배율 | 2.08x | 1.33x | **-36%** |
| 선명도 | 97 | 97 | 동등 |
| 파일 크기 | 472 KB | 472 KB | 동등 |

**참고**: 설정 블록 +0x02 (photo_res)는 해상도를 제어하지 않음 — 0x00~0x04 전부 1600x1200 출력.
→ 상세: `problem1/README.md`

**코드 영역 패치 가능 확인 (Problem 2 해결)**:
- 코드 영역(0x01xxxx) 내 문자열 1바이트 변경 → SD 업그레이드 → 정상 부팅 ✅
- 이전 720p(1280x720) 패치 brick은 코드 보호가 아닌 4:3 파이프라인에 16:9 주입한 런타임 문제
→ 상세: `problem2/README.md`

#### 4-1. ISP 레지스터 파라미터 튜닝 (진행 중)

**ISP 파라미터 설정 함수 발견** (0x01BF80~0x01C060):

두 분기(Block A/B)가 `[R7+16..19]`에 ISP 파라미터를 STRB:

| 오프셋 | Block A | Block B | 추정 파라미터 |
|--------|---------|---------|-------------|
| [R7+16] | 77 (0x4D) | 88 (0x58) | 파라미터 1 |
| [R7+17] | 74 (0x4A) | 86 (0x56) | 파라미터 2 |
| [R7+18] | 80 (0x50) | 73 (0x49) | 파라미터 3 |
| [R7+19] | 71 (0x47) | 68 (0x44) | 파라미터 4 |
...
|------|----------|-------------|------|
| 선명도 | 97 | **138** | **+42%** (최고치) |
| Chroma Cb/Cr | 15.7/16.5 | **14.2/15.1** | **-8~10%** |
| Saturation | 109 | 125 | +14% |

→ [R7+17]은 크로마 디노이즈 전용이 아닌 **ISP 전반 처리 강도**에 영향.
→ 나머지 3개 파라미터([R7+16], [R7+18], [R7+19]) 개별 테스트 필요.

**파일 오프셋 매핑 (Block A)**:
- [R7+16]: `0x1BFB8` (MOV R2, #0x4D)
- [R7+17]: `0x1BFBC` (MOV R2, #0x4A) ← 테스트 완료
- [R7+18]: `0x1BFC4` (MOV R2, #0x50)
- [R7+19]: `0x1BFCC` (MOV R2, #0x47)

#### 4-2. 감마 커브 커스터마이징

ISP 하드웨어가 RGB 감마 LUT(256엔트리) 지원 확인됨 (GPCV2247F 사양).
- 펌웨어에서 감마 테이블 초기화 코드 찾기 (256엔트리 루프, CMP #256 패턴)
- sRGB 표준 감마(2.2) 또는 커스텀 톤 커브 적용
- 암부 디테일 살리기 (WDR 유사 효과)

#### 4-3. Color Correction Matrix (CCM) 튜닝

3x3 색보정 매트릭스 계수 조정:
- 센서 특성에 맞춘 색재현 개선
- 녹색 색조(tint) 보정
- ISP 초기화 코드에서 9개 고정소수점 계수 찾기
...
Phase 4 (고급 ISP)       ████████      🔄 진행 중 — CMP스와핑 1280x960 성공, ISP [R7+17] 테스트 완료, 나머지 3개 파라미터 남음
